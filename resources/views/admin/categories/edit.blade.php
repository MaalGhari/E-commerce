    ...
    <div class="d-flex align-items-center justify-content-center min-vh-100">
        ...
        <div class="p-4 w-100 max-w-md">
            ...
            <div class="bg-white rounded-lg shadow bg-light">
                ...
                <form class="p-4 bg-light" action="{{ route('admin.dashboard.categories.update', $category->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row g-4 mb-4">
                        <div class="col-12">
                            <label for="" class="form-label mb-2">Name</label>
                            <input type="text" name="name" class="form-control" placeholder="" value="{{ $category->name }}" required>
                        </div>
                        <div class="col-12">
                            <label for="description" class="form-label mb-2">Description</label>
                            <textarea name="description" id="description" class="form-control" rows="5" placeholder="Description" required>{{ $category->description }}</textarea>
                        </div>
                    </div>
